...
impl Computable for IfElse {
    fn compute(&self, args: &[Value], _: &Data) -> Value {
        match args.len() {
...
mod tests {
    ...
    fn simple() {
        ...
        assert_eq!(op.compute(&[], data), null);
        assert_eq!(op.compute(&[Value::Null], data), null);
    }
    ...
    fn one_arg() {
        ...
        assert_eq!(op.compute(&[json!(true)], data), json!(true));
        assert_eq!(op.compute(&[json!(false)], data), json!(false));
        assert_eq!(op.compute(&[json!("foo")], data), json!("foo"));
    }
    ...
    fn two_args() {
        ...
        assert_eq!(op.compute(&[json!(true), json!(5)], data), json!(5));
        assert_eq!(op.compute(&[json!(false), json!(5)], data), json!(null));
        assert_eq!(
            op.compute(&[json!("foo"), json!("bar")], data),
            json!("bar")
        );
    }
    ...
    fn three_args() {
        ...
        assert_eq!(
            op.compute(&[json!(true), json!(5), json!(6)], data),
            json!(5)
        );
        assert_eq!(
            op.compute(&[json!(false), json!(5), json!(6)], data),
            json!(6)
        );
    }
    ...
    fn more() {
        ...
        assert_eq!(
            op.compute(&[json!(false), json!(5), json!(true), json!(6)], data),
            json!(6)
        );
        assert_eq!(
            op.compute(&[json!(false), json!(5), json!(false), json!(6)], data),
            json!(null)
        );
        assert_eq!(
            op.compute(
